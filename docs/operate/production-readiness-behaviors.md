...
| 49 | Subagent add | A canonical subagent file exists in a supported source. | Run `agents subagents add <source> --subagent <name> -y`. | The canonical Markdown is stored once, lockfile metadata is recorded, and every eligible harness receives its native rendered format. |
