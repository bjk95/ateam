...
| 22 | Matching directory auto-heal | A real directory exists at a target skill path and matches the canonical snapshot byte-for-byte. | Run `agents apply`. | The directory is replaced with the expected symlink without needing `--force`. |
