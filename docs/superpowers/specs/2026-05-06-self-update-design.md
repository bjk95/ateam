...
Add a Claude-Code-style auto-updater to `ateam` so users on machines that already have the binary get new releases without re-running the installer script by hand.

Two surfaces:

1. **Implicit:** every `ateam <cmd>` invocation silently checks for updates at most once per 24 hours and installs them in place.
2. **Explicit:** a new `ateam upgrade` subcommand bypasses the TTL and runs the same check/install path with output.
...
ateam <any-cmd>
   ...
   │     ├─ read mtime of ~/.cache/ateam/.update-check
   │     ├─ if (now - mtime) < 24h    → return [warm cache, ~1 syscall]
   │     └─ else (cold cache):
   │          ├─ axoupdater.run_sync()
   │          │    ├─ Ok(Some(result))  → log "ateam: 0.1.0 → 0.2.0", touch cache
...
`ateam upgrade` skips the TTL check and runs the same `axoupdater.run_sync()` path with output enabled. It returns a real `Result` so a manual upgrade exits non-zero on failure — the user explicitly asked us to update, they want to know if it broke.
...
2. Soft-fail is the only failure mode in `maybe_check`. The check has no path to make `ateam <cmd>` fail.
...
pub fn force_upgrade() -> Result<()>;  // called from `ateam upgrade`, prints output
```

**Internals (private):**

- `cache_path() -> Result<PathBuf>` → `~/.cache/ateam/.update-check`. Resolved via `directories::BaseDirs::cache_dir()` (XDG cache, transient by convention) so it is **outside** the git-managed `~/.config/ateam/` repo and never shows up in `git status` or auto-sync commits.
- `ensure_cache_dir(path: &Path) -> Result<()>` → `create_dir_all` on the parent. `~/.cache/ateam/` may not exist yet on a fresh machine; we own creating it.
...
The guard prevents `ateam upgrade` from running the silent TTL-gated check before the explicit upgrade — they'd both call `axoupdater.run_sync()` and the silent one would race the loud one for the cache file.
...
fn build_updater() -> AxoUpdater {
    let mut u = AxoUpdater::new_for("ateam");
    ...
    let _ = (|| -> Result<()> {
        ...
        match u.run_sync() {
            Ok(Some(result)) => {
                ...
                eprintln!("ateam: updated {} → {}", result.old_version, result.new_version);
                touch_cache(&cache)?;
            }
...
pub fn force_upgrade() -> Result<()> {
    ...
    match u.run_sync()? {
        Some(result) => println!("ateam: updated {} → {}", result.old_version, result.new_version),
        None         => println!("ateam: already at latest ({})", env!("CARGO_PKG_VERSION")),
    }
...
| `~/.cache/ateam` missing | `ensure_cache_dir` creates it; if `create_dir_all` itself fails, swallow | Same |
| Pointer / repo not yet initialized (`ateam init` never run) | Irrelevant — cache lives outside the repo, no dependency on `paths::resolve_repo()` | Same |
...
| Latest release missing `ateam-installer.sh` | Swallow, cache untouched | Bubble up, exit 1 |
...
`maybe_check` runs *before* `git_sync.pull`. The reasoning: putting it after `git_sync` means a busted install could prevent a `git pull`. Putting it first means the worst case is "update succeeded but the user's command crashed" — and the next `ateam` call gets the new binary, which is the right resolution.

## What this design explicitly does NOT include

- **Env-var opt-out.** No `ATEAM_NO_UPDATE_CHECK`. Soft-fail covers the offline/corporate/locked-down case. The user does not want to set environment variables for this.
...
1. `cargo run -- upgrade` against a real release where `CARGO_PKG_VERSION` is older than the latest tag. Expect: prints `updated X → Y`, binary at `~/.local/bin/ateam` is replaced.
2. `cargo run -- list` with wifi off. Expect: command succeeds, no error printed, no hang.

### CI

No new CI. cargo-dist's existing release pipeline already publishes `ateam-installer.sh` per release; that's the artifact `axoupdater` consumes. If the installer asset ever stops being published, the failure surfaces on the first user `upgrade` — not something a unit test would catch better.

## Documentation impact

- `README.md` — add a one-paragraph "Updating" section near "Install", documenting the auto-check behavior and the explicit `ateam upgrade` command.
...
- `ateam doctor` — already an explicit non-goal in `WISHLIST.md`.
